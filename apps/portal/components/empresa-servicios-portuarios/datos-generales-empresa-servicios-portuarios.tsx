...
import { CategoriaServiciosPortuarios } from "./categoria-servicios-portuarios"
...
export default function DatosGeneralesEmpresaServiciosPortuarios({ 
  onSave, 
  onNext, 
  onSaveDraft, 
  onSubmit, 
  ...
  const [formData, setFormData] = useState({
    ...
    categoriasServiciosPortuarios: [] as {
      grupo: string
      tareas: string[]
    }[],
  ...
  const handleMultipleFileAdd = (tipo: "referenciasFinancieras" | "balancesAprobados") => {
    const newItem: DocumentoArchivo = {
  ...
  const handleMultipleFileChange = (
    ...
    setFormData((prev) => ({
      ...
      [tipo]: prev[tipo].map((item) =>
        item.id === id ? { ...item, archivo: file } : item
      ),
    }))
  }
  ...
  const handleMultipleDateChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [tipo]: prev[tipo].map((item) =>
        item.id === id ? { ...item, fechaEmision: value } : item
      ),
    }))
  }
  ...
  const handleMultipleDateSelect = (date: Date, tipo: "referenciasFinancieras" | "balancesAprobados", id: string) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [tipo]: prev[tipo].map((item) =>
        item.id === id ? { ...item, fechaEmision: formattedDate } : item
      ),
    }))
    setOpenCalendar(null)
  }

  const handleCategoriasChange = (categorias: { grupo: string; tareas: string[] }[]) => {
    const newFormData = {
      ...formData,
      categoriasServiciosPortuarios: categorias,
    }
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); handleNext() }} className="space-y-6">
      ...
      {/* Sección de Categorías de Servicios Portuarios */}
      <div className="mt-8">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Categorías de Servicios Portuarios</h3>
        <CategoriaServiciosPortuarios 
          categorias={formData.categoriasServiciosPortuarios} 
