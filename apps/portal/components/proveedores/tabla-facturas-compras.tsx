...
interface TablaFacturasComprasProps {
  facturas: FacturaCompras[]
  onVerDetalle: (factura: FacturaCompras) => void
  onDescargar: (factura: FacturaCompras) => void
}

export function TablaFacturasCompras({ facturas, onVerDetalle, onDescargar }: TablaFacturasComprasProps) {
  const renderEstadoBadge = (estado: FacturaCompras["estado"]) => {
    const estadoConfig = {
    ...
    return <Badge className={estadoConfig[estado]}>{estado}</Badge>
  }
                  ...
                  <td className="px-4 py-4 text-sm text-center">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => onVerDetalle(factura)}>
